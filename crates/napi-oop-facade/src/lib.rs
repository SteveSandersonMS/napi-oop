...
//! (`Buffer`, `BigInt`, `External`, `ThreadsafeFunction`) with napi-oop's unified
//! equivalents. Those unified types implement BOTH the real napi traits (for the
//! in-proc ABI) and serde (for the out-of-proc wire), so one compiled function
//! works on both paths. Explicit named re-exports shadow the glob, so the
//! unified types win over the real napi ones.
